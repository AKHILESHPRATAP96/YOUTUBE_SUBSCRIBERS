...
//_id is used to display a specific subscriber from the database.
  app.get('/subscribers/:id', async (req, res) => {
    ...
    try {
      ...
      //When the subscriber for the search id cannot be discovered, an error message is displayed.
    } catch (error) {
      console.error(error);
  ...
  app.post('/subscribers/add', async (req, response) => {
    //adding a new subscriber as defined by the subscribemodel in the model
    ...
    try {
      ...
      response.send("Error in adding data to database");
    }
  });
...
//using the id, delete any subscriber 
  app.delete('/subscribers/delete/:id', async (req, res) => {
    ...
    try {
      ...
      res.send(`Subscriber deleted successfully ${subdel} `);
    } catch (err) {
      res.send(`Error in deleting data from database${delId}`);
    }
  });
